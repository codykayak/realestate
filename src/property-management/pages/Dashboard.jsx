...
/**
 * Operations dashboard — the ROI story in numbers. Every metric maps to a
 * money/time lever we pitch to operators (deflection, vacancy, fraud, labor).
 */
export default function Dashboard() {
  const { conversations, leasingLeads, workOrders, tenant } = usePm();
  ...
  const stats = useMemo(() => {
    ...
    // Rough time-saved model for the pitch: ~4 min saved per deflected inquiry +
    // ~25 min saved per auto pre-screen + ~30 min per self-help-deflected ticket.
    ...
    >
      ...
      <div className={`${styles.grid} ${styles.cols3}`}>
        <ValueCard
          ...
          body="Repetitive resident questions (rent due, pool hours, packages, pet policy) are answered instantly from the property knowledge base, 24/7 — reclaiming staff hours and covering nights/weekends at zero marginal cost."
        />
        <ValueCard
          icon="key"
          title="Faster, safer leasing"
          body="Instant lead response plus automated pre-screening and application auditing cut vacancy days and keep bad/fraudulent applications from ever reaching a human."
        />
        <ValueCard
          icon="wrench"
          title="Maintenance fast-track"
          body="AI triages every request, flags emergencies immediately, and deflects fixable issues with guided self-help — fewer truck rolls, faster closes, happier residents."
        />
      </div>
      ...
      <div className={styles.banner} style={{ marginTop: 22 }}>
        ...
        <div>
          <strong>This is a live build-and-pitch demo.</strong> All modules run on local demo data so you can click
          through the full experience. Connect your PMS, Twilio, and screening providers in <strong>Settings →
          Integrations</strong>, and point the module at a new Firebase project to make it multi-user and persistent.
        </div>
      </div>
    </Page>
  );
